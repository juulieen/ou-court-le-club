...
|---|---|---|---|---|
| **Klikego** | `klikego.py` | Per-department iteration: `GET /v8/evenements/search.jsp?geo={dept}&sport=running` over all depts (01–95, 97, 98) | v8: `GET /inscrits/{slug}/{ref}?heat={heatId}&city={club}` — **iterate every heat** (distance, from `el-select[name=heat]`), server-side club filter per heat, 0-indexed pagination. Falls back to v6 POST `findInInscrits.jsp` | Name-based server search is broken (500), so **club-field matching only** (name-only members with empty club are missed — downloading all registrants of ~436 events blows the scrape budget). Unfiltered discovery caps at ~120 nearest events & ignores `page` → iterate `geo` per dept (~436 events). A v8 event page renders only its FIRST heat unless `heat` is passed |
| **Njuko** | `njuko.py` | Persistent slug cache (`njuko_slugs.json`) seeded from **Common Crawl CDX index** (every run, free, no key) + Wayback CDX fallback + `_SEED_SLUGS`; validates each slug via API | REST API: `/edition/url/{slug}` then **`POST /registrations/{id}`** (paginated, `criteria:null` for bulk; some editions reject bulk with **403 "Criteria are required"** → per-name search fallback). **Club field removed from public API → name-matching only** (`known_members`). Large events (> `BIG_EVENT_THRESHOLD`) use per-name search instead of bulk | Also handles 4 white-labels: UTMB (`register-utmb.world`), Sporkrono (`sporkrono-inscriptions.fr`), Sports107 (`sports107.com`), timeto (`timeto.com`). Old GET `/_search/{}` endpoint is now **410 Gone** |
...
| `njuko_slugs.json` | Persistent Njuko event slug cache. Seeded from Common Crawl CDX index (every run) with Wayback Machine CDX as fallback; grows over time. Format: `{slugs: ["slug1", ...]}`. |
